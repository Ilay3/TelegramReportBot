    ...
    public class StatisticsService : IStatisticsService
    {
        ...
        public async Task<byte[]> ExportStatisticsAsync(string format, DateTime? from = null, DateTime? to = null)
        {
            ...
            switch (format.ToLowerInvariant())
            {
                ...
                case "csv":
                    return System.Text.Encoding.UTF8.GetBytes(GenerateCsvReport(report));

                case "txt":
                    return System.Text.Encoding.UTF8.GetBytes(GenerateTextReport(report));
        ...
        /// </summary>
        public async Task<List<Models.FileInfo>> GetTopFilesAsync(string criteria, int count = 10)
        {
            lock (_lockObject)
            {
        ...
        private List<Models.FileInfo> GetLargestFiles(List<FileProcessingResult> files, int count)
        {
